/**
 * Task #883 — Regression test for the sweep-threshold CSV export
 * `GET /api/admin/production-assets/orphan-sweep/threshold/history.csv`.
 *
 * Locks in the behavior layered onto this route by Tasks #853, #873,
 * and #879 before someone refactors the row formatter:
 *
...
const ACTOR_A = "task-883-actor-alpha";
const ACTOR_B = "task-883-actor-beta";
const ACTOR_C = "task-883-actor-gamma";
...
function appWithStubAdmin() {
  ...
  const requireAdmin: express.RequestHandler = (req: any, _res, next) => {
    req.session = {
      isAdmin: true,
      adminActorId: "task-883-stub-admin",
...
async function getCsv(qs: string) {
  const res = await fetch(
    `${base}/api/admin/production-assets/orphan-sweep/threshold/history.csv${qs}`,
  );
...
test("csv: default (no timeZone) emits UTC columns + UTC header + UTC filename", async () => {
  ...
  assert.match(disp, /attachment;\s*filename="orphan-sweep-threshold-history-.*-UTC\.csv"/);
...
test("csv: valid timeZone surfaces in column, header, filename; local != UTC", async () => {
  ...
  assert.match(disp, /attachment;\s*filename="orphan-sweep-threshold-history-.*-America_Los_Angeles\.csv"/);
...
test("csv: invalid timeZone falls back to UTC instead of 400-ing", async () => {
  const { status, headers, text } = await getCsv(
    `?timeZone=${encodeURIComponent("Not/A_Real_Zone")}`,
  );
  assert.equal(status, 200);
  assert.equal(headers.get("x-export-time-zone"), "UTC");
  const disp = headers.get("content-disposition") || "";
...
test("csv: from/to filters narrow the row set + X-Export-Row-Count", async () => {
  ...
  const newValues = rows.slice(1).map((r) => r[5]).sort();
  assert.deepEqual(newValues, ["30", "40"]);
});
...
test("csv: actorUserId filter narrows to that actor's rows only", async () => {
  ...
  assert.equal(rows.length, 1 + 2);
  for (const r of rows.slice(1)) {
    assert.equal(r[6], ACTOR_A);
  }
  const newValues = rows.slice(1).map((r) => r[5]).sort();
  assert.deepEqual(newValues, ["10", "20"]);
});
