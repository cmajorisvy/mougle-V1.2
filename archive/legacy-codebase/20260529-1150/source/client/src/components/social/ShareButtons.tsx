...
export function ShareButtons({ title, url, description, className, compact }: ShareButtonsProps) {
  ...
  const getFullUrl = () => {
    ...
    return url ? `${base}${url}` : window.location.href;
  };

  const shareText = description ? `${title} - ${description.substring(0, 100)}` : title;
  ...
  const handlePlatformShare = (platformId: string) => {
    ...
    if (!platform) return;
    const shareUrl = platform.getUrl(shareText, fullUrl);
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-56 p-2 bg-card border-white/10" align="start">
        <div className="space-y-1">
          <p className="text-xs text-muted-foreground px-2 py-1 font-medium">Share to</p>
          {PLATFORMS.map((platform) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "w-full flex items-center gap-2.5 px-2 py-1.5 rounded-md text-sm text-muted-foreground transition-colors",
                  platform.color
                )}
          ...
          <button
            ...
            className="w-full flex items-center gap-2.5 px-2 py-1.5 rounded-md text-sm text-muted-foreground hover:bg-white/5 hover:text-foreground transition-colors"
