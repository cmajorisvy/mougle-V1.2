...
type DetailResponse = { ok: boolean; asset: Asset };

export default function AssetSafetyReview() {
  const [, params] = useRoute<{ id: string }>("/admin/3d-assets/:id/safety-review");
  const id = params?.id ?? "";
  ...
  useEffect(() => {
    ...
    setLicenseStatus(data.asset.licenseStatus || "unknown");
    setLicenseSource(data.asset.licenseSource ?? "");
    setLicenseNote(data.asset.licenseNote ?? "");
    setSafetyNote(data.asset.safetyNote ?? "");
    if (data.asset.safetyReview && data.asset.safetyReview !== "pending") {
      setDecision(data.asset.safetyReview);
    }
  }, [data?.asset]);
  ...
  async function submit() {
    ...
    try {
      const licRes = await apiRequest(
        "POST",
        `/api/admin/production-assets/${id}/license`,
        {
          licenseStatus,
      ...
      const safRes = await apiRequest(
        "POST",
        `/api/admin/production-assets/${id}/safety-review`,
        {
          decision,
      ...
      setTimeout(() => navigate(`/admin/3d-assets/${id}`), 600);
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground" data-testid="page-3d-assets-safety-review">
      <div className="mx-auto max-w-3xl px-4 py-6">
        <div className="mb-4 flex items-center justify-between gap-3">
          <Link href={`/admin/3d-assets/${id}`}>
          ...
          <CardHeader>
            ...
            {data?.asset && (
              <p className="mt-1 text-sm text-muted-foreground">
                Asset: <span className="text-foreground">{data.asset.name}</span> ·{" "}
                <Badge variant="outline" data-testid="pill-current-safety">{data.asset.safetyReview}</Badge>{" "}
                <Badge variant="outline" data-testid="pill-current-gate">{data.asset.approvalGate}</Badge>
              </p>
            )}
          </CardHeader>
          <CardContent>
